...
tweets = LOAD 'src/test/resources/hdfsbiohistorytestin.txt' USING PigStorage('\t') AS (id: chararray, source: chararray, timestamp: long, object: chararray);
...
streamsbios = FOREACH millibios GENERATE
    ...
    millis AS timestamp,
    bio_json AS object;
STORE streamsbios INTO 'target/timestampedbios' USING PigStorage('\t');
